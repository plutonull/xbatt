...
# operating system:  Linux 6.1.0-37-686-pae i686 [ELF] (6.1.0)
# libc:	(6.36.0)
# binutils:	(240)
...
install:: xbatt
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTPGMFLAGS)  xbatt $(DESTDIR)$(BINDIR)/xbatt

all:: xbatt.$(MANNEWSUFFIX)

xbatt.$(MANNEWSUFFIX): xbatt.$(MANSRCSUFFIX)
	$(RM) $@
	if test -z "$(PREPROCESSMANPAGES)" ; then \
	   cd `dirname xbatt` && \
	   $(LN) `basename xbatt.$(MANSRCSUFFIX)` `basename $@`; \
	else \
	      	     $(RAWCPP)  $(MANDEFS) $(EXTRAMANDEFS) \
	     < xbatt.$(MANSRCSUFFIX) | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' 			-e '/^#line  *[0-9][0-9]*  *.*$$/d' 			-e '/^[ 	]*XCOMM$$/s/XCOMM/#/' 			-e '/^[ 	]*XCOMM[^a-zA-Z0-9_]/s/XCOMM/#/' 			-e '/^[ 	]*XHASH/s/XHASH/#/' 			-e '/\@\@$$/s/\@\@$$/\\/' >$@; \
	fi

cleandir::
	$(RM) xbatt.$(MANNEWSUFFIX)

install.man:: xbatt.$(MANNEWSUFFIX)
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) xbatt.$(MANNEWSUFFIX) $(DESTDIR)$(MANDIR)/xbatt.$(MANSUFFIX)
